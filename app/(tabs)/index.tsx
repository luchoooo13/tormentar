...
export default function HomeScreen() {
  ...
  const regionalCurrentAlert = useMemo(() => {
    ...
    return (
      getNearestRegionPoint(location.latitude, location.longitude, regionPoints)
        ?.alert ?? null
    );
  }, [location, regionPoints]);

  const homeAlerts = useMemo(() => {
    const visibleAlerts = filterAlertsByMinSeverity(
      filteredAlerts,
      preferences.minSeverity,
    );
    const regionalAllowed =
      regionalCurrentAlert &&
      enabledSeverities.includes(regionalCurrentAlert.severity)
        ? regionalCurrentAlert
        : null;

    if (!regionalAllowed) return visibleAlerts;

    const existingIndex = visibleAlerts.findIndex(
      (alert) => alert.id === regionalAllowed.id,
    );
    if (existingIndex < 0) return [regionalAllowed, ...visibleAlerts];

    const existing = visibleAlerts[existingIndex];
    if (
      SEVERITY_ORDER[regionalAllowed.severity] >=
  ...
  const renderAlertCard = (alert: WeatherAlert) => {
    ...
    return (
      <View
        key={alert.id}
        className="mb-3 p-4 rounded-2xl border-l-4"
        style={{
      ...
      >
        ...
        {weather?.current && (
          <View className="px-5 py-4">
            <View
              className="bg-surface p-5 rounded-3xl border"
            ...
            >
              <View className="flex-row items-center justify-between">
                ...
                <AnimatedWeatherIcon
                  weatherId={weather.current.weather?.[0]?.id ?? 800}
                  size={68}
                />
              </View>
            </View>
        ...
        {!hasApiKey && (
          <View className="px-5 py-4">
            ...
            >
              ...
              <Text className="text-xs text-muted">
                No se pudo habilitar el servicio meteorologico. Verifica tu
                conexion a internet; la app usa Open-Meteo (gratuito, sin API
                key), por lo que no requiere configuracion adicional.
              </Text>
            </View>
          </View>
        )}
        ...
        <View className="px-5 py-4">
          ...
          <Text className="text-xs text-muted mt-2">
            ...
            {preferences.minSeverity === "severa"
              ? "fuerte"
              : preferences.minSeverity}{" "}
            en adelante. Se aplica en toda la app (Inicio, Mapa y
            notificaciones).
          </Text>
        </View>
        ...
        <View className="px-5 py-5">
          ...
          {error && hasApiKey && (
            <View
              className="bg-surface p-4 rounded-xl border mb-3"
            ...
            !error && (
              ...
              >
                ...
                <Text className="text-xs text-muted text-center mt-1">
                  No hay alertas activas para los niveles seleccionados en tu
                  zona.
                </Text>
              </View>
            )
