...
async function saveSubscription(sub) {
  ...
  await db.insert(pushSubscriptions).values(values).onDuplicateKeyUpdate({ set: { p256dh: values.p256dh, auth: values.auth } });
}
...
var pushRouter = router({
  ...
  notify: publicProcedure.input(
    z2.object({
      title: z2.string(),
      body: z2.string(),
      severity: z2.string().optional(),
      alertId: z2.string().optional(),
...
function startKeepAlive() {
  ...
  const ping = async () => {
    try {
      ...
      console.log(`[keep-alive] ping ${res.status} a las ${(/* @__PURE__ */ new Date()).toLocaleTimeString()}`);
    } catch (err) {
      console.warn("[keep-alive] fallo el ping:", err instanceof Error ? err.message : err);
    }
  };
  console.log(`[keep-alive] activado, pingueando ${healthUrl} cada ${PING_INTERVAL_MS / 6e4} min`);
